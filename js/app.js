...
(function () {
  ...
  // Short "first few words…" label for a long quote — used anywhere a
  // quote needs to act as a title (Favorites list, the WhatsApp comment
  // link, share text) instead of dumping the entire quote in as a title.
  function quoteSnippet(text, wordCount) {
    const words = String(text || '').trim().split(/\s+/);
    if (words.length <= (wordCount || 8)) return String(text || '').trim();
    return words.slice(0, wordCount || 8).join(' ') + '…';
  }
  ...
  // One-time startup check: if two poems (or quotes/books) ever end up
  // with the same id — e.g. a multi-part poem's "part_of" value in the
  // workbook happens to match another poem's own poem_id — findPoem()
  // above will silently always return the FIRST match, which is exactly
  // what "clicking one poem opens a different one" looks like from the
  // outside. This can't be fixed from here (it's a workbook data issue,
  // not a code bug), but it logs a clear, specific warning naming the
  // colliding titles so it's easy to track down and fix in the workbook.
  (function checkForDuplicateIds() {
    function reportDupes(list, label) {
      const seen = new Map();
      list.forEach(item => {
        if (!item.id) return;
        if (seen.has(item.id)) {
          ...
          seen.set(item.id, item.title || item.text || '');
        }
      });
    }
    reportDupes(DATA.poems, 'poems');
    reportDupes(DATA.quotes, 'quotes');
    reportDupes(DATA.books, 'books');
  })();
  ...
  // Delegated click-to-navigate — safer than baking IDs into inline
  // onclick="jtkiazNav('...')" strings, which breaks (and can silently
  // navigate to the wrong item) if any id/title ever contains a quote
  // character. data-navto goes through the exact same HTML-attribute
  // escaping (esc()) already used for every other data-* attribute in
  // this file, so it can't be broken out of, regardless of content.
  document.addEventListener('click', (e) => {
    const el = e.target.closest('[data-navto]');
    if (!el) return;
  ...
  function mountWhatsNewOverlay() {
    ...
    // Belt-and-braces on top of the CSS max-height: cap the panel's
    // height using the ACTUAL measured viewport in pixels (window.
    // innerHeight), recalculated on resize/orientation-change. CSS vh
    // units are notoriously unreliable on mobile — they don't always
    // account for the browser's address bar the same way across every
    // phone/browser — so this is a second, more direct guarantee that
    // the panel (and its close button) can never grow taller than what
    // the user can actually see on screen.
    function capHeight() {
      panel.style.maxHeight = Math.round(Math.min(window.innerHeight * 0.7, 620)) + 'px';
    }
    capHeight();
    ...
    function dismissForVisit() {
      ...
      window.removeEventListener('resize', capHeight);
    ...
    // Escape-key dismiss as a safety net — belt-and-braces in case the
    // close button is ever hard to reach (e.g. an unusually short
    // viewport), there's still a guaranteed way to get the panel gone.
    function onKey(e) { if (e.key === 'Escape') { dismissForVisit(); document.removeEventListener('keydown', onKey); } }
  ...
  // The real, plain, "open this in the actual app/site" link for a track
  // — used as a visible fallback next to every SoundCloud/Audiomack
  // iframe. Third-party embedded players are known to silently fail to
  // load in a lot of mobile contexts that have nothing to do with this
  // app's own code: Safari's tracking-prevention blocking third-party
  // iframe cookies, and — very relevant here, since this app links out
  // to WhatsApp constantly — WhatsApp's/Instagram's/Facebook's built-in
  // "in-app browser" webviews, which are well known for breaking
  // embedded players that work fine in a normal mobile browser. A plain
  // link that opens the track directly always works regardless.
  function trackOpenLink(t) {
    if (t.source === 'soundcloud') {
      // if src is already a widget URL, the real page link is the
      // ?url= parameter inside it; otherwise src IS the page link
      const m = /[?&]url=([^&]+)/.exec(t.src || '');
      return m ? decodeURIComponent(m[1]) : (t.src || '').split('?')[0];
    }
    return (t.src || '').split('?')[0];
  }
  ...
  function mountPlayerSurface(track) {
    ...
    const openHref = esc(trackOpenLink(track));
    const platform = track.source === 'soundcloud' ? 'SoundCloud' : 'Audiomack';
    const fallback = `<a class="track-embed-fallback" href="${openHref}" target="_blank" rel="noopener">↗ Open in ${platform}</a>`;
    if (track.source === 'local' && track.src) {
      ...
      // No loading="lazy" here — some mobile browsers (notably iOS Safari
      // and various in-app WebViews) have had long-standing bugs where a
      // lazily-loaded CROSS-ORIGIN iframe like this SoundCloud/Audiomack
      // embed never actually fires its load, leaving a blank space where
      // the player should be. It's one small iframe, so eager-loading it
      // costs little and makes it reliably show up on every device.
      surface.innerHTML = `<iframe src="${esc(soundcloudEmbedSrc(track.src, true))}" allow="autoplay" title="${esc(track.title)}"></iframe>${fallback}`;
    } else if (track.source === 'audiomack' && !isPlaceholderSrc(track.src)) {
      // (was missing the loading="lazy" removal applied to the SoundCloud
      // branch above — same bug, same fix, just caught late.)
      surface.innerHTML = `<iframe src="${esc(audiomackEmbedSrc(track.src, true))}" allow="autoplay" title="${esc(track.title)}"></iframe>${fallback}`;
    } else {
  ...
  function renderHome() {
    ...
    root.innerHTML = `
      <section class="hero wrap">
        ...
        <div class="hero-stats">
          ...
          <button class="hero-stat" data-navto="poems" aria-label="See all poems"><strong data-count-to="${DATA.poems.length}">0</strong><span>Poems</span></button>
          <button class="hero-stat" data-navto="quotes" aria-label="See all quotes"><strong data-count-to="${DATA.quotes.length}">0</strong><span>Quotes</span></button>
          <button class="hero-stat" data-navto="music" aria-label="See all tracks"><strong data-count-to="${DATA.music.length}">0</strong><span>Tracks</span></button>
        </div>
        ${galleryBooks.length ? `
        <div class="hero-gallery">
          ${galleryBooks.map(b => `
            <figure data-navto="books/${esc(b.id)}" style="cursor:pointer">
      ...
      <div class="wrap">
        <div class="quote-of-day scroll-quote">
          ...
          <blockquote data-navto="quotes/${esc(qod.id)}" style="cursor:pointer">"${esc(qod.text)}"</blockquote>
          <cite>— ${esc(qod.origin)}</cite>
          <div class="foot">
            <button class="copy-btn" data-text="${esc(qod.text)} — ${esc(qod.origin)}">Copy</button>
            ${favBtnHTML('quote', qod.id, quoteSnippet(qod.text), qod.origin)}
    ...
    wireFavButtons(root);
    wireHeroStatCounters(root);
  }

  // Counts each hero stat up from 0 to its real value on load — a small
  // "odometer" flourish for the digital-styled stat numbers above.
  // Respects prefers-reduced-motion (jumps straight to the final number
  // for anyone who's asked their device to minimize animation).
  function wireHeroStatCounters(container) {
  ...
  function bookListRowHTML(b) {
    return `
      <div class="book-list-row" data-navto="books/${esc(b.id)}" style="cursor:pointer">
  ...
  function bookCardHTML(b) {
    ...
    return `
      <div class="book-card" style="--book-accent:${esc(b.themeColor || '')}">
        <div class="book-cover" data-navto="books/${esc(b.id)}" style="cursor:pointer">
          ${b.cover ? `<img src="${esc(coverSrc(b.cover))}" alt="${esc(b.title)}" loading="lazy" onerror="this.remove()">` : ''}
          <span class="type-tag">${esc(b.type)}</span>
          ${favBtnCornerHTML('book', b.id, b.title, b.type)}
        </div>
        <div class="book-body">
          <h3 data-navto="books/${esc(b.id)}" style="cursor:pointer">${esc(b.title)}</h3>
          ...
          <div class="card-actions">
            <button class="btn ghost" data-navto="books/${esc(b.id)}">Open</button>
      ...
      <div class="wrap view" style="--book-accent:${esc(book.themeColor || '')}">
        ...
        <div class="chapter-list">
          ${book.chapters.map(c => {
            ...
            return `
              <div class="chapter-row ${isUnlocked ? 'unlocked' : 'locked'}" ${isUnlocked ? `data-navto="books/${esc(book.id)}/${c.number}"` : ''}>
  ...
  function renderChapterReader(book, num) {
    ...
    root.innerHTML = `
      <div class="wrap view" style="--book-accent:${esc(book.themeColor || '')}; ${fontDecl}">
        <span class="back-link" data-navto="books/${esc(book.id)}">&larr; ${esc(book.title)}</span>
        ${readerToolbarHTML()}
        ...
        <div class="reader framed">
          ...
          <div class="reader-actions">
            <button class="btn ghost" data-navto="books/${esc(book.id)}">All chapters</button>
            ${next
              ? `<button class="btn accent" data-navto="books/${esc(book.id)}/${next.number}">Next chapter → (unlocked)</button>`
  ...
  function poemCardHTML(p) {
    ...
    return `
      <div class="poem-card" style="--poem-accent:${esc(p.color || '')}">
        <div class="poem-thumb" data-navto="poems/${esc(p.id)}" style="cursor:pointer">
        ...
        <div class="poem-body">
          <div data-navto="poems/${esc(p.id)}" style="cursor:pointer">
            <h3>${esc(p.title)}</h3>
  ...
  function poemListRowHTML(p) {
    ...
    const img = poemImageFor(p);
    return `
      <div class="poem-list-row" data-navto="poems/${esc(p.id)}">
  ...
  let ambientAudioSrc = ''; // which poem's sound file is currently loaded — see below
  function wirePoemSound(poem) {
    const btn = root.querySelector('#poem-sound-btn');
    if (!btn || !poem.sound) return;
    const src = coverSrc(poem.sound);
    btn.addEventListener('click', () => {
      // A single shared <audio> element is reused across poems (so
      // switching poems doesn't leave old sounds playing in the
      // background) — but it must be reloaded whenever the poem's sound
      // file actually changes, otherwise clicking Play on a NEW poem
      // just resumes whatever the PREVIOUS poem's sound was. That was
      // the "wrong material" bug: `if (!ambientAudio)` only ever created
      // the Audio object once, so every later poem reused the first
      // poem's audio, regardless of `src`.
      if (ambientAudio && ambientAudioSrc === src) {
        if (!ambientAudio.paused) { ambientAudio.pause(); btn.textContent = '▶ Play ambient sound'; return; }
      } else {
      ...
      <div class="wrap view">
        ...
        <div class="quote-of-day scroll-quote">
          ...
          <div class="foot">
            <button class="copy-btn" data-text="${esc(qod.text)} — ${esc(qod.origin)}">Copy</button>
            ${favBtnHTML('quote', qod.id, quoteSnippet(qod.text), qod.origin)}
            <button class="ghost-btn" data-navto="quotes/${esc(qod.id)}">Open →</button>
          </div>
          <div class="scroll-band"></div>
          <div class="scroll-rod bottom"></div>
        </div>` : ''}
  ...
  function quoteCardHTML(q) {
    return `
      <div class="quote-card" data-navto="quotes/${esc(q.id)}" style="cursor:pointer">
        <p class="text">"${esc(q.text)}"</p>
        <div class="foot">
          <cite>— ${esc(q.origin)}</cite>
          <div style="display:flex;align-items:center;gap:0.3rem">
            ${favBtnHTML('quote', q.id, quoteSnippet(q.text), q.origin)}
  ...
  function quoteRowHTML(q) {
    return `
      <div class="quote-row" data-navto="quotes/${esc(q.id)}" style="cursor:pointer">
  ...
  function renderQuoteDetail(id) {
    ...
    updateFooterCta('quotes', quoteSnippet(q.text));
    const n = quoteNavNeighbors(q.id);
    const order = quoteNavOrder && quoteNavOrder.length ? quoteNavOrder : DATA.quotes;
    ...
    root.innerHTML = `
      <div class="wrap view">
        ...
        ${engageBarHTML('quote', q.id, quoteSnippet(q.text))}
  ...
  function trackEmbedIframeHTML(t) {
    ...
    const openHref = esc(trackOpenLink(t));
    const platform = t.source === 'soundcloud' ? 'SoundCloud' : 'Audiomack';
    // No loading="lazy" — see the note above wireTrackPlayer's SoundCloud
    // iframe; the Music tab is exactly where this bug hits hardest, since
    // every track below the first one sits off-screen until scrolled to.
    // The "Open in SoundCloud/Audiomack" link below is a real, always-
    // working fallback for the cases the iframe itself can't reliably
    // cover on mobile (see trackOpenLink's comment above).
    return `<div class="track-embed"><iframe src="${esc(src)}" allow="autoplay" scrolling="no" frameborder="no" title="${esc(t.title)}"></iframe><a class="track-embed-fallback" href="${openHref}" target="_blank" rel="noopener">↗ Open in ${platform}</a></div>`;
  }
    ...
    root.innerHTML = `
      <div class="wrap view">
        ...
        ${mode === 'grid'
          ...
          : `<div class="blog-list">
              ${DATA.blog.map(b => `
                <div class="blog-row" data-navto="blog/${esc(b.id)}" style="cursor:pointer">
                  <div>
  ...
  function blogCardHTML(b) {
    return `
      <div class="blog-card" data-navto="blog/${esc(b.id)}" style="position:relative">
